...
def pypi_top100(executor: Executor) -> Iterator[list[PyPIPackage]]:
    ...
    if not expired:
        yield pickle.loads(_cache_path.read_bytes())
    else:
        ...
        if response.status_code != 200:
            ...
            pytest.fail(msg)
        stats = PyPIStatData.model_validate(response.json())
        ...
        for package_name, response in zip(stats.top_packages, responses):
            if response.status_code != 200:
                log.warning("Failed to get PyPI package data for %s.", package_name)
